...
import { ADD_TO_CART, REMOVE_ITEM, SUB_QUANTITY, ADD_QUANTITY, ADD_SHIPPING, REMOVE_COMPARE_PRODUCT, ADD_COMPARE_PRODUCT } from '../actions/action-types/cart-actions'
...
const initState = {
  items: [
    ...
    },
    {
      ...
      "type": "dairy",
      "description": "Glass of green smoothie with quail egg's yolk, served with cocktail tube, green apple and baby spinach.",
...
const cartReducer = (state = initState, action) => {
  ...
  if (action.type === ADD_TO_CART) {
    let addedItem = state.items.find((item) => { return item.title === action.title })
  ...
  if (action.type === ADD_COMPARE_PRODUCT) {
    let addedCompareItem = state.items.find((item) => { return item.title === action.title })
    let existed_item = state.compareItems.find(item => action.title === item.title)
    if (existed_item) {
      return { ...state }
    }
    else {
      return {
        ...state,
        compareItems: [...state.compareItems, addedCompareItem]
      }
    }
  }
  if (action.type === REMOVE_COMPARE_PRODUCT) {
    let new_items = state.compareItems.filter(item => action.title !== item.title)
    return {
      ...state,
      compareItems: new_items,
    }
  }
